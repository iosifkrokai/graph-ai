...
async def create_node(
    data: Annotated[NodeCreate, Body(default=...)],
    session: Annotated[AsyncSession, Depends(db.get_session)],
    usecase: Annotated[NodeUsecase, Depends(node_dependency.get_node_usecase)],
) -> NodeResponse:
    """Create a node."""
    return await usecase.create_node(session=session, data=data)
...
async def list_nodes(
    ...
    return await usecase.get_nodes(session=session, workflow_id=workflow_id)
...
async def get_node(
    ...
    """Fetch a node by ID."""
    return await usecase.get_node(session=session, node_id=node_id)


@router.patch(path="/{node_id}")
async def update_node(
    node_id: Annotated[int, Path(default=..., gt=0)],
    data: Annotated[NodeUpdate, Body(default=...)],
    session: Annotated[AsyncSession, Depends(db.get_session)],
    usecase: Annotated[NodeUsecase, Depends(node_dependency.get_node_usecase)],
) -> NodeResponse:
    """Update a node by ID."""
    return await usecase.update_node(session=session, node_id=node_id, data=data)
