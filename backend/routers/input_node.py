...
async def create_input_node(
    data: Annotated[InputNodeCreate, Body(default=...)],
    session: Annotated[AsyncSession, Depends(db.get_session)],
    usecase: Annotated[
        InputNodeUsecase, Depends(input_node_dependency.get_input_node_usecase)
    ],
) -> InputNodeResponse:
    """Create an input node configuration."""
    return await usecase.create_input_node(session=session, data=data)
...
async def get_input_node(
    ...
    return await usecase.get_input_node(session=session, node_id=node_id)


@router.patch(path="/{node_id}")
async def update_input_node(
    node_id: Annotated[int, Path(default=..., gt=0)],
    data: Annotated[InputNodeUpdate, Body(default=...)],
    session: Annotated[AsyncSession, Depends(db.get_session)],
    usecase: Annotated[
        InputNodeUsecase, Depends(input_node_dependency.get_input_node_usecase)
    ],
    ...
    return await usecase.update_input_node(session=session, node_id=node_id, data=data)
